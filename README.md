# Лабораторная работа №7: Консольное приложение (CLI) 🖥️


## Цель работы

- Объединить все знания из ЛР1–ЛР6 в единое работающее приложение.
- Реализовать интерактивный CLI-интерфейс с меню и обработкой ввода.
- Освоить слоистую архитектуру, собственные исключения, сохранение/загрузку данных.

## Описание CLI

Меню включает пункты:
1. Добавить героя (выбор типа, ввод имени, уровня, ауры/маны)
2. Показать всех героев (таблица)
3. Найти героя по имени
4. Удалить героя (с подтверждением)
5. Фильтрация (только воины / только маги / только живые)
6. Сортировка (по имени, уровню, ауре, мане)
7. Статистика
0. Выход (с автоматическим сохранением)

Ошибки ввода обрабатываются (ValueError, собственные исключения).

## Демонстрация работы

### Сценарий 1: Запуск и загрузка данных

![Загрузка](./src/images/lab07/1.png)

При запуске приложение загружает коллекцию из `data.json`. Если файла нет – создаёт новую.

### Сценарий 2: Добавление и удаление с подтверждением

![Добавление](./src/images/lab07/2.1.png)
![](./src/images/lab07/2.2png)
![](./src/images/lab07/2.3png)

- Добавление воина и мага.
- Попытка добавить дубликат → ошибка `1`.
- Удаление с запросом `y/n`.

### Сценарий 3: Сортировка

![Сортировка](./src/images/lab07/3.1.png)
![](./src/images/lab07/3.2.png)

Выбор стратегии сортировки из меню, вывод после сортировки.

### Сценарий 4: Фильтрация и обработка исключений

![Фильтрация](./src/images/lab07/4.1.png)
![](./src/images/lab07/4.2.png)

- Фильтрация по типу.
- Поиск несуществующего героя → `HeroNotFoundError`.


# Лабораторная работа №6: Generics и typing 🧩

## Цель работы

- Освоить систему аннотаций типов в Python (typing).
- Научиться создавать обобщённые (generic) классы с помощью TypeVar и Generic.
- Понять концепцию структурной типизации через Protocol.

---

## Реализованные типы и контейнеры

### Аннотации типов

Все классы (`Character`, `Voin`, `Mag`, `TypedCollection`) снабжены полными аннотациями типов: параметры конструктора, атрибуты, возвращаемые значения.

### Generic-коллекция `TypedCollection[T]`

Обобщённая версия коллекции из ЛР-2. Параметризуется типом элементов `T`. Содержит методы:
- `add(item: T) -> None`
- `remove(item: T) -> None`
- `get_all() -> list[T]`
- `find(predicate: Callable[[T], bool]) -> Optional[T]`
- `filter(predicate: Callable[[T], bool]) -> list[T]`
- `map(transform: Callable[[T], R]) -> list[R]`

### Протоколы (структурные интерфейсы)

- **`Displayable`** – требует метод `display() -> str`.
- **`Scorable`** – требует метод `score() -> float`.

Классы `Voin` и `Mag` имеют эти методы (без явного наследования). TypeVar с ограничением `D = TypeVar('D', bound=Displayable)` гарантирует, что в `TypedCollection[D]` можно положить только объекты, поддерживающие `display()`.

---

## Демонстрация работы

### Сценарий 1: `TypedCollection[Displayable]`

![Сценарий 1](./src/images/lab06/1.png)

- Создаётся коллекция, которая принимает объекты с методом `display()`.
- Добавлены `Voin` и `Mag` (у обоих есть `display()`).
- Демонстрируются методы `find`, `filter`, `map`.

### Сценарий 2: `TypedCollection[Scorable]`

![Сценарий 2](./src/images/lab06/2.png)

- Коллекция принимает объекты с методом `score()`.
- Добавлены воин и маг.
- Показаны поиск максимального `score` и преобразование через `map`.

---


# Лабораторная работа №5: Функции как аргументы. Стратегии и делегаты. 🧠🎯

## Цель работы

- Освоить передачу **функций как аргументов** в другие функции и методы.
- Научиться применять встроенные функции высшего порядка: `map`, `filter`, `sorted`.
- Понять концепцию паттерна «Стратегия» и реализовать его на Python.
- Освоить `lambda`-выражения и их практическое применение.
- Интегрировать функциональный стиль с объектно-ориентированным кодом из предыдущих ЛР.

---

## Реализованные стратегии и функции

Все стратегии вынесены в отдельный файл `strategies.py`.

### Стратегии сортировки

- `by_name(obj)` – ключ сортировки: имя (алфавитный порядок)
- `by_level(obj)` – ключ сортировки: уровень (числовой)
- `by_aura(obj)` – ключ сортировки: аура (у воинов), у магов возвращает 0
- `by_mana(obj)` – ключ сортировки: мана (у магов), у воинов возвращает 0
- `make_sort_by_attr(attr_name)` – фабрика, возвращает функцию для сортировки по любому атрибуту

### Функции-фильтры

- `is_alive(obj)` – оставляет только живых персонажей
- `is_warrior(obj)` – оставляет только объекты класса `Voin`
- `is_mage(obj)` – оставляет только объекты класса `Mag`
- `make_min_level_filter(min_level)` – фабрика, возвращает фильтр «уровень >= min_level»
- `make_max_health_filter(max_health)` – фабрика, возвращает фильтр «здоровье <= max_health»

### Callable‑стратегии (классы с `__call__`)

- `SortByAttribute(attr_name)` – сортировка по произвольному атрибуту (хранит имя атрибута)
- `FilterByType(class_type)` – фильтрация по типу класса (например, `FilterByType(Voin)`)
- `AddExperienceStrategy(amount)` – добавляет персонажу заданное количество опыта
- `HealStrategy()` – полностью лечит персонажа

### Методы коллекции `CharacterTeam` (добавлены в `collection.py`)

- `sort_by(key_func, reverse=False)` – сортирует коллекцию на месте, возвращает `self`
- `filter_by(predicate)` – возвращает новую коллекцию с отфильтрованными элементами
- `apply(func)` – применяет функцию к каждому элементу, возвращает `self`
- `map_to(transform_func)` – возвращает список результатов преобразования

---

## Демонстрация работы

Все сценарии находятся в файле `demo.py`. Скриншоты в папке `src/images/lab05/`.

### Сценарий 1: Сортировка разными стратегиями и фильтрация

![Сценарий 1](./src/images/lab05/1.png)

- Создаётся коллекция из 6 персонажей (3 воина, 3 мага).
- Применяются три стратегии сортировки: по имени, по уровню, по ауре (убывание).
- Фильтрация: только воины (`is_warrior`), только высокоуровневые (через фабрику `make_min_level_filter`).

### Сценарий 2: Использование `map`, фабрик и `lambda`

![Сценарий 2](./src/images/lab05/2.png)

- `map` для получения списка имён персонажей.
- `map` с лямбдой для получения строки `"уровень: имя"`.
- Применение фабрики фильтров `make_min_level_filter` для отбора персонажей с уровнем ≥ 4.
- Лямбда-фильтр по здоровью > 100.

### Сценарий 3: Цепочка операций и callable-стратегии

![Сценарий 3](./src/images/lab05/3.png)

- Создаётся новая коллекция.
- Строится цепочка: `filter_by(только воины) → sort_by(по ауре, убывание) → apply(+50 опыта)`.
- Применяется `HealStrategy` ко всем персонажам.
- Демонстрируется, что `filter_by` возвращает новую коллекцию, а исходная не изменяется.



# Лабораторная работа №4: Интерфейсы и абстрактные классы — Voin и Mag (Вариант 6) 🎯

## Цель работы

- Познакомиться с **абстрактными базовыми классами (ABC)**
- Освоить понятие **интерфейса (контракта поведения)**
- Научиться **задавать обязательные методы для классов**
- Закрепить **полиморфизм через единый интерфейс**
- Научиться **проектировать архитектуру, а не просто классы**

---

## Реализация в классах

Классы **`Voin`** и **`Mag`** (из ЛР-3) теперь наследуют не только `Character`, но и оба интерфейса.

### Класс `Voin` (Воин)

- Реализует `to_string()`: выводит имя, уровень, ауру, мотивацию.
- Реализует `compare(other)`: сравнивает двух воинов **по ауре**.

### Класс `Mag` (Маг)

- Реализует `to_string()`: выводит имя, уровень, ману, сонливость.
- Реализует `compare(other)`: сравнивает двух магов **по мане**.

Таким образом, один и тот же интерфейс (`Comparable`) работает по-разному для разных классов — это **полиморфизм**.

---

## Демонстрация работы

### Сценарий 1: Интерфейс Printable и универсальная функция `print_all`

![Сценарий 1](./src/images/lab04/1.png)

- Создаются воины `Красавчик Ян` и `Супер воин`, маги `Жуков Никита` и `Мудрый маг`.
- Вызывается метод `to_string()` напрямую у воина и мага.
- Универсальная функция `print_all()` получает список из разных объектов и печатает их через `to_string()` — единый интерфейс.

### Сценарий 2: Интерфейс Comparable и проверка типов через `isinstance()`

![Сценарий 2](./src/images/lab04/2.png)

- Проверка, что воин и маг реализуют `Printable` и `Comparable` (через `isinstance`).
- Сравнение двух воинов по ауре: вызов `compare()` возвращает результат.
- Сравнение двух магов по мане: вызов `compare()` показывает, у кого маны больше.

### Сценарий 3: Интеграция с коллекцией – фильтрация по интерфейсу

![Сценарий 3](./src/images/lab04/3.png)

- Все персонажи добавляются в коллекцию `CharacterTeam`.
- Метод `get_printable()` возвращает новую коллекцию только из объектов, реализующих `Printable`.
- Метод `get_comparable()` возвращает новую коллекцию только из объектов, реализующих `Comparable`.
- Выводятся строковые представления отфильтрованных объектов.

---

# Лабораторная работа №3: Наследование и иерархия классов — Voin и Mag (Вариант 6) 🎮⚔️

## Цель работы

- Освоить механизм **наследования классов**
- Научиться строить **иерархию объектов**
- Понять разницу между базовым и производным классами
- Научиться переиспользовать код через `super()`
- Освоить **переопределение методов** и **полиморфизм**


## Реализованная иерархия классов

В качестве базового класса используется **`Character`** из ЛР-1.  
Дочерние классы **`Voin`** и **`Mag`**.

### Класс `Voin` (Воин)

**Новые атрибуты:**
- `aura` — аура
- `motivazia` — мотивация

**Новые методы:**
- `kill_mag(target)` — убить мага
- `attack_mag(target)` — атаковать мага

**Переопределённые методы:**
- `make_sound()` — боевой клич воина
- `use_skill(target)` — использование способности

### Класс `Mag` (Маг)

**Новые атрибуты:**
- `mana` — мана
- `sleep` — сонливость

**Новые методы:**
- `kill_voin(target)` — убить воина
- `attack_voin(target)` — атаковать воина

**Переопределённые методы:**
- `make_sound()` — боевой клич мага
- `use_skill(target)` — использование способности

---

## Демонстрация работы

### Сценарий 1: Создание персонажей и вывод информации

![Создание](./src/images/lab03/1.png)

- Создание воина `Voin` с именем "Красавчик Ян", уровень 5, аура 50, мотивация 30
- Создание мага `Mag` с именем "Жуков Никита", уровень 5, мана 100
- Вывод информации через `__str__`

### Сценарий 2: Полиморфизм (make_sound)

![Полиморфизм](./src/images/lab03/2.png)

- Вызов `make_sound()` у воина: "Красавчик Ян: Кто последний, тот отчислен!"
- Вызов `make_sound()` у мага: "Жуков Никита: Кто первый, тот красавчик!"
- Один и тот же метод — разное поведение

### Сценарий 3: Атаки

![Атаки](./src/images/lab03/3.png)

- Воин атакует мага методом `attack_mag()` — наносит урон, повышает ауру
- Маг атакует воина методом `attack_voin()` — тратит ману, наносит урон
- Отслеживание изменения здоровья

### Сценарий 4: Убийства

![Убийства](./src/images/lab03/4.png)

- Воин убивает мага методом `kill_mag()` — наносит 9999 урона, повышает ауру и мотивацию
- Маг убивает воина методом `kill_voin()` — наносит 9999 урона, повышает ману и сонливость
- Персонаж после убийства переходит в состояние `dead`

### Сценарий 5: Полиморфизм use_skill и проверка типов

![Полиморфизм use_skill](./src/images/lab03/5.png)

- Вызов полиморфного метода `use_skill(target)` у воина и мага
- Проверка типов через `isinstance()`: воин является `Voin` и `Character`, но не `Mag`; маг является `Mag` и `Character`, но не `Voin`


# Лабораторная работа №2: Коллекция объектов — CharacterTeam (Вариант 6) 🎮⚔️

## Цель работы

- Научиться работать с **коллекциями объектов**
- Понять разницу между **моделью сущности** и **контейнером объектов**
- Реализовать **собственный контейнерный класс**
- Освоить **итерацию по объектам** (`__iter__`, `__len__`, `__getitem__`)
- Реализовать базовые операции управления коллекцией (добавление, удаление, поиск, сортировка, фильтрация)

---

## Реализованная коллекция

В качестве элементов коллекции используется класс **`Character`** из ЛР-1 (игровой персонаж).  
Контейнерный класс **`CharacterTeam`** управляет группой персонажей.

### Класс `CharacterTeam` (коллекция)

**Атрибуты:**
- `_items` — внутренний список объектов `Character`.

## Демонстрация работы

### Сценарий 1: Создание коллекции, добавление, удаление, вывод

![Создание](./src/images/lab02/1.png)

- Создаются персонажи `Арагорн`, `Леголас`, `Гимли`, `Гэндальф`.
- Добавление в коллекцию `CharacterTeam`.
- Проверка вывода всех элементов.
- Удаление одного персонажа (`Гимли`).
- Попытка добавить дубликат (по имени) — генерируется `ValueError`.
- Попытка добавить объект неправильного типа — `TypeError`.

### Сценарий 2: Поиск, длина, итерация

![Поиск, длина, итерация](./src/images/lab02/2.png)

- Поиск по имени `find_by_name("Леголас")` возвращает объект.
- Поиск по уровню `find_by_level(4)` возвращает список персонажей уровня 4.
- `len(team)` — количество элементов.
- Цикл `for c in team:` — перебор всех персонажей.

### Сценарий 3: Индексация и удаление по индексу

![Индексация и удаление по индексу](./src/images/lab02/3.png)

- Доступ по индексу: `team[0]` — первый персонаж, `team[1]` — второй.
- Срезы: `team[1:3]` возвращает подсписок.
- Удаление по индексу: `remove_at(1)` удаляет элемент с индексом 1.
- Обработка выхода за границы (`IndexError`).

### Сценарий 4: Сортировка

![Сортировка](./src/images/lab02/4.png)

- `sort_by_level()` — сортировка по уровню (по возрастанию).
- `sort_by_name(reverse=True)` — сортировка по имени (по убыванию).
- Универсальная сортировка: `team.sort(key=lambda c: c.health)`.

### Сценарий 5: Фильтрация (логические операции)

![Фильтрация](./src/images/lab02/5.png)

- `get_alive()` — возвращает **новую коллекцию** только с живыми персонажами.
- `get_by_min_level(5)` — возвращает коллекцию персонажей с уровнем ≥ 5.
- Фильтрация не изменяет исходную коллекцию.


# Лабораторная работа №1: Игровая логика — класс Character (Вариант 6) 🎮⚔️

![photo](https://user-images.githubusercontent.com/74038190/235224431-e8c8c12e-6826-47f1-89fb-2ddad83b3abf.gif)

## Цель работы

- Освоить объявление пользовательских классов
- Разобраться с инкапсуляцией (закрытые поля)
- Реализовать свойства (`@property`)
- Переопределить магические методы (`__str__`, `__repr__`, `__eq__`)
- Понять разницу между атрибутами класса и экземпляра

**Задумка**  
Создать игрового персонажа (Character), который:

- Следит за корректностью данных (валидация имени, уровня, здоровья, опыта)
- Запрещает недопустимые операции (лечение мёртвого, повышение уровня выше максимума)
- Предоставляет удобный интерфейс через свойства (`@property`)
- Имеет состояние (`alive`/`dead`), влияющее на доступные действия

---

## Реализованный класс

**Character**

```python
class Character:
    
    MAX_LEVEL = 100
    BASE_HEALTH = 100
    HEALTH_PER_LEVEL = 15
```

## Атрибуты класса:
MAX_LEVEL — максимально достижимый уровень

BASE_HEALTH — базовое здоровье на 1 уровне

HEALTH_PER_LEVEL — прирост здоровья за уровень

## Закрытые поля:+_name — имя персонажа
+ _level — уровень
+ _health — текущее здоровье
+ _experience — опыт
+ _state — состояние (alive/dead)

## Свойства @property:
+ name (чтение) — имя персонажа
+ name (чтение и запись) — сеттер с валидацией
+ level (чтение) — уровень
+ health (чтение) — текущее здоровье
+ experience (чтение) — опыт

+ state (чтение) — состояние

+ max_health (вычисляемое, чтение) — максимальное здоровье в зависимости от уровня

## Магические методы:
+ str — для print (читаемое описание)

+ repr — для разработчиков

+ eq — сравнение по имени и уровню

## Бизнес-методы:
+ take_damage(amount) — получение урона

+ heal(amount) — лечение

+ gain_experience(amount) — получение опыта
  
+ level_up() — повышение уровня

attack(target) — атака другого персонажаив ли персонаж (если нет — выбрасывают RuntimeError). Также проверяются корректность входных данных (тип, диапазон).git 

### Сценарий 1: Создание персонажей

![Создание персонажей](./src/images/lab01/1.png)

+ Создание персонажа с полными параметрами (Арагорн, уровень 5, здоровье 150, опыт 250)
+ Создание персонажа с параметрами по умолчанию (Новичок)
+ Создание нескольких персонажей с разными характеристиками

Показывает, что каждый персонаж получает свои уникальные характеристики, а значения по умолчанию применяются корректно.
  
### Сценарий 2: Валидация (обработка ошибок)

![Валидация (обработка ошибок)](./src/images/lab01/2.png)

+ Некорректные имена (пустые)
+ Некорректный уровень (0, 101 — превышение MAX_LEVEL)
+ Некорректное здоровье (отрицательное, больше максимума)

Программа намеренно пытается создать персонажей с некорректными данными, чтобы продемонстрировать защиту от ошибок.

### Сценарий 3: Свойства (геттеры/сеттеры)

![Свойства (геттеры/сеттеры)](./src/images/lab01/3.png)

+ Чтение всех свойств (имя, уровень, здоровье, опыт, статус)
+ Изменение имени владельца
+ Попытка изменения уровня напрямую (ошибка — уровень меняется только через методы)

Демонстрируется, как работать со свойствами класса — читать данные через геттеры и изменять через сеттеры с валидацией.

### Сценарий 4: Атрибуты класса

![Атрибуты класса](./src/images/lab01/4.png)

+ Доступ к атрибутам класса через сам класс (Character.MAX_LEVEL)
+ Доступ к атрибутам класса через экземпляр (hero.MAX_LEVEL)

Показывается разница между атрибутами класса (общие для всех) и атрибутами экземпляра (индивидуальные).

### Сценарий 5: Боевые механики

![Боевые механики](./src/images/lab01/5.png)

+ Получение урона (здоровье уменьшается)
+ Лечение (здоровье восстанавливается, но не выше максимума)
+ Получение опыта
+ Повышение уровня при накоплении достаточного опыта

Демонстрируются основные RPG-механики и проверка граничных значений.

### Сценарий 6: Смерть и ограничения

![Смерть и ограничения](./src/images/lab01/6.png)

+ Смертельный урон (здоровье становится 0, статус меняется на "dead")
+ Попытка лечить мёртвого персонажа (ошибка)
+ Попытка дать опыт мёртвому персонажу (ошибка)

Прослеживается жизненный цикл персонажа — смерть блокирует все дальнейшие действия.

### Сценарий 7: Атака между персонажами

![Атака между персонажами](./src/images/lab01/7.png)

+ Атака одного персонажа другим (урон = уровень × 10)
+ Смерть защитника от полученного урона
+ Попытка атаковать уже мёртвого персонажа (ошибка)

Демонстрируется взаимодействие двух объектов — передача урона и изменение состояния.

### Сценарий 8: Повышение уровня и максимальный уровень

![Повышение уровня и максимальный уровень](./src/images/lab01/8.png)

+ Получение опыта, приводящее к нескольким повышениям уровня подряд
+ Попытка повысить уровень выше максимального (ошибка)

Показывается, что уровень не может превысить атрибут класса MAX_LEVEL.

### Сценарий 9: Магические методы

![Магические методы](./src/images/lab01/9.png)

str (пользовательский вывод):
+ Создаётся персонаж
+ Выводится результат print(character)
+ На экране появляется читаемое описание персонажа
+ Это метод, который вызывается, когда объект нужно показать пользователю

repr (отладочный вывод):
+ Выводится результат repr(character)
+ Появляется строка типа Character('Арагорн', 5, 150.0, 250.0)
+ Это представление для отладки, показывающее, как создать такой же объект

eq (сравнение персонажей):
+ Сравниваем двух персонажей с одинаковыми именем и уровнем — True
+ Сравниваем с персонажем с другим именем — False
+ Сравниваем с объектом другого типа — False

Показать, что наши объекты можно выводить и сравнивать как встроенные типы Python.

### Сценарий 10: Проверка некорректных операций

![Проверка некорректных операций](./src/images/lab01/10.png)

+ Попытка нанести отрицательный урон (ошибка)
+ Попытка вылечить на отрицательную величину (ошибка)
+ Попытка дать отрицательный опыт (ошибка)

Демонстрируется, что все методы проверяют корректность входных данных и при ошибках персонаж остаётся в целостном состоянии.

